...
export default function WsConnectionStatusIndicator() {
  const { connectionState, errorMsg, sendMessage, closeConnection } = useWebSocket({
  ...
  return (
    <Suspense fallback={<p className="text-teal-300"> Loading la </p>}>
      ...
      <div className="text-center">
        ...
        <button className="p-2 py-1 rounded-md text-gray-500 text-center cursor-pointer bg-gray-700/50 hover:bg-gray-700 hover:text-white active:bg-gray-600 ml-1" onClick={closeConnection}>
          Close Connection
        </button>
      </div>
    </Suspense>
